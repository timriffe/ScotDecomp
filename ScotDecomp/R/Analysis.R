...
library(reshape2)
pik1981m <- acast(SCO[SCO$year == 1981 & SCO$sex == 1 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")
pik1981f <- acast(SCO[SCO$year == 1981 & SCO$sex == 2 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")

pik1991m <- acast(SCO[SCO$year == 1991 & SCO$sex == 1 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")
pik1991f <- acast(SCO[SCO$year == 1991 & SCO$sex == 2 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")

pik2001m <- acast(SCO[SCO$year == 2001 & SCO$sex == 1 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")
pik2001f <- acast(SCO[SCO$year == 2001 & SCO$sex == 2 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")

pik2011m <- acast(SCO[SCO$year == 2011 & SCO$sex == 1 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")
pik2011f <- acast(SCO[SCO$year == 2011 & SCO$sex == 2 & SCO$quintile_2 != 999, ], age ~quintile_2, value.var = "N")

# scale rows to 1
pik1981m <- pik1981m / rowSums(pik1981m)
pik1981f <- pik1981f / rowSums(pik1981f)

pik1991m <- pik1991m / rowSums(pik1991m)
pik1991f <- pik1991f / rowSums(pik1991f)

pik2001m <- pik2001m / rowSums(pik2001m)
pik2001f <- pik2001f / rowSums(pik2001f)

pik2011m <- pik2011m / rowSums(pik2011m)
pik2011f <- pik2011f / rowSums(pik2011f)
# calculate things manually (too much typing sorry)

# 1981
B1981m <- Vbetween(QX1981m, pik1981m)
W1981m <- Vwithin(QX1981m, pik1981m)

B1981f <- Vbetween(QX1981f, pik1981f)
W1981f <- Vwithin(QX1981f, pik1981f)

# 1991
B1991m <- Vbetween(QX1991m, pik1991m)
W1991m <- Vwithin(QX1991m, pik1991m)

B1991f <- Vbetween(QX1991f, pik1991f)
W1991f <- Vwithin(QX1991f, pik1991f)

# 2001
B2001m <- Vbetween(QX2001m, pik2001m)
W2001m <- Vwithin(QX2001m, pik2001m)

B2001f <- Vbetween(QX2001f, pik2001f)
W2001f <- Vwithin(QX2001f, pik2001f)

# 2011
B2011m <- Vbetween(QX2011m, pik2011m)
W2011m <- Vwithin(QX2011m, pik2011m)

B2011f <- Vbetween(QX2011f, pik2011f)
W2011f <- Vwithin(QX2011f, pik2011f)

# prop between
mpn <- cbind(B1981m / (B1981m + W1981m),
		B1991m / (B1991m + W1991m),
		B2001m / (B2001m + W2001m),
		B2011m / (B2011m + W2011m)
)
fpn <- cbind(B1981f / (B1981f + W1981f),
		B1991f / (B1991f + W1991f),
		B2001f / (B2001f + W2001f),
		B2011f / (B2011f + W2011f)
)
...
png("/home/tim/git/ScotDecomp/ScotDecomp/Figures/BetwweenAgeTimeRawpi.png",width=800,height=400)
par(mfrow=c(1,2))
matplot(a, mpn, type = 'l', col = gray(c(.6,.4,.2,0)),lwd = c(2,1.5,1.2,1),
		lty=1,
		ylab = "proportion",xlab = "age", ylim=c(0,.04),
		main = "Variance due to between-group differences, Males")
text(20,mp[21, ], c(1981,1991,2001,2011),pos=3,cex=.8)

matplot(a, fpn, type = 'l', col = gray(c(.6,.4,.2,0)),lwd = c(2,1.5,1.2,1),
		lty=1,
		ylab = "proportion",xlab = "age", ylim=c(0,.04),
		main = "Variance due to between-group differences, Females")
text(40,fp[41, ], c(1981,1991,2001,2011),pos=c(3,3,3,1),cex=.8)
...
mtn <- cbind((B1981m + W1981m),
		(B1991m + W1991m),
		(B2001m + W2001m),
		(B2011m + W2011m)
)
ftn <- cbind((B1981f + W1981f),
		(B1991f + W1991f),
		(B2001f + W2001f),
		(B2011f + W2011f)
)
...
par(mfrow=c(1,2))
matplot(a, sqrt(mtn), type = 'l', col = gray(c(.6,.4,.2,0)),lwd = c(2,1.5,1.2,1),
		lty=1,
		ylab = "sd",xlab = "age",ylim=c(0,16),
		main = "Total sd, Males")

matplot(a, sqrt(ftn), type = 'l', col = gray(c(.6,.4,.2,0)),lwd = c(2,1.5,1.2,1),
		lty=1,
		ylab = "proportion",xlab = "age",ylim=c(0,16),
		main = "Total sd, Males")
