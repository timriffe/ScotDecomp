...
of death is patterned when measuring socioeconomic inequality by area-level
deprivation even though it has long been established that area of residence is
an important part of the explanation for socioeconomic differences in mortality
outcomes (Carstairs and Morris 1989; Macintyre, Ellaway and Cummins 2002).
Area-level measures aim to capture relative deprivation rather than absolute
...
for survival (Carstairs and Morris 1989; Kearns, Gibb and Mackay 2000; Townsend 1987).
 
Two processes underly variance in age at death: Individual
stochasticity (within-group variation) and heterogeneity (between-group
inequality). Within group inequality arises from variance due to random
...
variance in age of death can be interpreted as individual stochasticity. Between-group inequality can arise if individuals at the same age are subject to different mortality rates that may be due to unobserved social, economic or environmental contexts (Hartemink, Missov and Caswell 2017).
...
age at death (van Raalte et al. 2012). This study used aggregated data for the
...
over time due to compositional changes.  In addition, stratifying data by
education or other acquired states forces the researcher to left-truncate data
at some age, such a 35.
...
(Look for Arun Hendi ``Trends in US life expectancy gradients: the role of
changing educational composition'', he uses something education quintiles,
similar motivation)
...
in society (Carstairs and Morris 1990). This means the Carstairs score is a method of capturing relative deprivation at the contextual level.
...
110 (Wilmoth et al. 2007).\footnote{Specifically, we apply equations (53) and
...
variability applied to the distribution of age at death (van Raalte and Caswell 2013). Details of the within-group and between-group component calculations are given in the following subsection.

Rosie: note that standard deviations make more sense when truncating. At age
zero they aren't considered the best. May want to mention it. I wonder if this
same within-between notion works for $e^\dagger$? Food for thought.


\subsection{Variance decomposition}
Lifetable variance is calculated and decomposed into within- and between-group
components using matrix algebra expressions. From the lifetable, we extract
conditional single-age death probabilities, $q_x$, and take its complement,
$p_x$. We then calculate the projection matrix for the $k^{th}$
subpopulation, $\mathbf{U}_k$ as:
\begin{equation}
\mathbf{U}_k = 
\begin{bmatrix}
...
These may aggregate to a total lifetable for the entire population according to
some mixing distribution, $\pi$.
...
mixtures may be based on an age-pattern to $\pi$ from the stationary
population produced by each quintile lifetable. By this method, lower
mortality lifetables result in greater relative weight in older ages. In the
case of stationary weighting, weights are assigned only to the lifetable
radix. In our case, the subpopulations are aggregated into deprivation quitiles,
and they end up being of approximately equal size. Therefore the uniform radix
mixing distribution is acceptable.\footnote{In practice we calculated all
results with both raw population weights and age-specific stationary population
weights, and these turn out to be almost identical. Decomposition results are
shown only sing stationary weights, as these were somewhat smoother.}
...
the highest variation in age at death at each year. For males there was an
...
deprived quintile have experienced decreasing variation in age at death but the
decrease was far greater for the least deprived. The change in standard
...
very little over this 40-year range.
...
of health measures in Scotland (NHS: Public Health and Intelligence. 2017).
